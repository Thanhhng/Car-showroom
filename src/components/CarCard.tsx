...
import CarDetail from "./CarCardDetail";
...
export default function CarCard({ data,myRef }:any) {
    ...
    return (
        <div className="grid 2xl:grid-cols-4 xl:grid-cols-3 md:grid-cols-2 grid-cols-1 w-full gap-8 pt-14 mb-8 px-8" ref={myRef}>
            {Array.isArray(data) 
                ? data.map((e, index) => (
                    ...
                    <div className='relative w-full h-40 my-3 object-contain'>
                        <Image src={getImage(e)}  alt="car-pic" fill={true} sizes="" priority className="object-contain" />
                    </div>
            ...
            {isOpen && createPortal(
                <CarDetail  handleClick={handleChangeProps} stateProps={isOpen} carData={data ? data[indexOfItem] : null} />,
                document.body,
            )}
        </div>
    );
